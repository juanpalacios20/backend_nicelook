...
@api_view(['PUT'])
def update_review(request, client_id, employee_id, appointment_id):
    ...
    try:
        ...
        if not ReviewEmployee.objects.filter(autor=autor, appointment=appointment).exists():
            return Response({'error': 'You have not reviewed this appointment'}, status=status.HTTP_400_BAD_REQUEST)
        
        review = ReviewEmployee.objects.get(autor=autor, appointment=appointment)
